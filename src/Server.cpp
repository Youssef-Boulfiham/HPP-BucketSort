...
//! list
std::string Server::getListOfLists(const std::vector<std::vector<int>> &listOfLists) {
...
int Server::run(const std::string &taskName,
    ...
    BucketSort bucketSort = BucketSort(taskName);
    // int a = bucketSort.getPositiveDigitsCount({1});
    bucketSort.run(listUnsorted);
    // cout << a << endl;
    return 0;
}
